...
- 2026-07-12: Expanded the real VS Code host smoke to pass, warning, and blocked trusted fixtures; shortened disposable profile paths for macOS IPC limits and kept all profiles outside the repository.
- 2026-07-12: Replaced the generic VS Code incomplete-check warning with actionable summaries for no staged changes, failed tests, failed quality checks, and missing coverage; host assertions cover each outcome.
- 2026-07-12: Split readiness message and coverage formatting into `preCrCheckMessages.ts`, keeping the VS Code pre-check feature below the source-size quality threshold without changing its public behavior.
